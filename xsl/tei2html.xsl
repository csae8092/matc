...
<!-- taken from https://git.rz.uni-jena.de/thulb/collections -->
  <xsl:template match="tei:TEI">
    ...
    <html>
      <head>
        ...
        <link rel="stylesheet" type="text/css" href="{$WebApplicationBaseURL}css/webfont-junicode.css"/>
        <link rel="stylesheet" type="text/css" href="{$WebApplicationBaseURL}css/tei.css"/>
        <title>
  ...
  <!-- The actual text of the footnote at the bottom of the page -->
  <xsl:template match="tei:p[contains(@rend, 'footnote')]" mode="footnote">
    <xsl:comment>
      Start - match="tei:p[contains(@rend, 'footnote')]" mode="footnote" (TEI-tei2html.xsl)
    </xsl:comment>
    ...
    <xsl:comment>
      End - match="tei:p[contains(@rend, 'footnote')]" mode="footnote" (TEI-tei2html.xsl)
    </xsl:comment>
  </xsl:template>
  ...
  <xsl:template match="tei:ref">
    <xsl:choose>
      ...
      <xsl:when test="@source='urmel_collections'">
        <a>
          <xsl:attribute name="href">
            <xsl:text>https://collections.thulb.uni-jena.de/rsc/viewer/</xsl:text>
            <xsl:value-of select="@target"/>
          </xsl:attribute>
